<div
    ...
    @if ($this->canSpin)
        <button type="button" wire:click="$dispatch('openCashoutModal')" :disabled="{{ !$this->canCashOut }}"
            class="transition-all bg-stone-500 hover:bg-stone-600 disabled:opacity-40 text-white px-16 py-4 rounded-md text-lg">Cash
            Out</button>
    @else
    ...
    <button type="button" wire:click="$dispatch('spin')" :disabled="{{ !$this->canSpin }}"
